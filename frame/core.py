...
def multi_process_frame(source_path: str, temp_frame_paths: List[str], process_frames: Callable[[str, List[str], Any], None], update: Callable[[], None]) -> None:
    with ThreadPoolExecutor(max_workers=swapper.globals.execution_threads) as executor:
        ...
        queue_per_future = max(len(temp_frame_paths) // swapper.globals.execution_threads, 1)
...
def update_progress(progress: Any = None) -> None:
    ...
    progress.set_postfix({
        ...
        'execution_providers': swapper.globals.execution_providers,
        'execution_threads': swapper.globals.execution_threads
    })
    progress.refresh()
    progress.update(1)
